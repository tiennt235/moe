...
def render_roster_list(roster: Roster) -> str:
    return "\n".join(
        f"- **{e.name}** (`moe-{e.slug}`) — {e.description.strip()}" for e in roster.experts
    )


def expert_body(expert: ExpertSpec, knowledge_path: str, root: Path) -> str:
    tmpl = _load(_TEMPLATES / "expert-behavior.md")
    body = (
...
def router_skill(roster: Roster, delegation: str) -> str:
    body = _load(_SKILL_SRC / "SKILL.md")
    return body.replace("{{ROSTER}}", render_roster_list(roster)).replace(
        "{{DELEGATION}}", delegation
    )


def _skill_description(roster: Roster) -> str:
    domains = ", ".join(e.name for e in roster.experts)
    return (
...
def build_claude_code(roster: Roster, root: Path, dist: Path) -> None:
    ...
    (skill_dir / "SKILL.md").write_text(
        _frontmatter([("name", "moe"), ("description", _skill_description(roster))])
        + router_skill(roster, CC_DELEGATION)
    )
...
def build_codex(roster: Roster, root: Path, dist: Path) -> None:
    ...
    (skill_dir / "SKILL.md").write_text(
        _frontmatter([("name", "moe"), ("description", _skill_description(roster))])
        + router_skill(roster, CODEX_DELEGATION)
    )
    _copy_commands(skill_dir)
    ...
    (base / "AGENTS.moe.md").write_text(_agents_snippet(roster))
...
def build_generic(roster: Roster, root: Path, dist: Path) -> None:
    ...
    (skill_dir / "SKILL.md").write_text(
        _frontmatter([("name", "moe"), ("description", _skill_description(roster))])
        + router_skill(roster, GENERIC_DELEGATION)
        + "\n"
        + "\n".join(inline)
        + "\n"
    )
...
def _agents_snippet(roster: Roster) -> str:
    return (
...
def write_plugin_manifest(roster: Roster, root: Path) -> None:
    ...
    manifest = {
        "name": "moe",
        "version": __version__,
        "description": _skill_description(roster),
        "author": "moe",
...
def build_all(roster: Roster, root: Path) -> list[str]:
    ...
    return ["claude-code", "codex", "agents"]
